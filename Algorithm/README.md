...
__최대값을 찾을 때, x값은 문제가 해결되는지? 예. -> x값을 늘림 -> x+1값은 문제가 해결되는지? 아니오. -> x값이 최대값

__최소값을 찾을 때, x값은 문제가 해결되는지? 예. -> x값을 줄임 -> x-1값은 문제가 해결되는지? 아니오. -> x값이 최소값

위 과정을 반복하면 x는 문제가 해결되고, x+1은 문제 해결이 안되는 값을 찾을 수 있다.
...
parametric_search(min, max){
	...
	x = (min + max)/2; // 범위 조절, x값을 정의
	
	count = 0; // 결정 판단을 위한 값
	for(rope_length : rope_list) count += rope_length / x; // x로 몇개의 줄을 만들 수 있는지
	
	// m개를 만들 수 있다 -> x값을 늘림 (최대값 찾기)
	// m개를 만들 수 없다 -> x값을 줄임
	if(count >= m) return parametric_search(x+1, max);
...
응용 문제같은 경우는 파라메트릭 서치 적용이 어렵기보다는 애초에 해당 알고리즘을 적용시키면 되겠다는 아이디어 떠올리기가 아주 어렵다..
